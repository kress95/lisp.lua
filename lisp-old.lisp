  ...
  (= expand-body
     (function [ast macros]
      ...
      (for-in [tail head] [(pairs ast)]
        (= out (form-cons (expand head macros) out)))
...
(do
  ...
  ;;; flattens vararg into a table
  (local output)
  (do
    (local output-next)

    ;;; iterates vararg until item is nil
    (= output-next
      (function [out idx item ...]
        (if [(== item nil) (return out)])
        (if [(and (== (type item) "table") (== (getmetatable item) nil))
             (for-in [_ sub-item] [(ipairs item)]
              (= idx (+ idx 1))
              (= (at out idx) sub-item))]
            [else
              (= idx (+ idx 1))
              (= (at out idx) item)])
        (return (output-next out idx ...))))

    ;; flattens vararg into a table
    (= output
      (function [...]
        (return (output-next {table} 0 ...)))))

  (local indent)
  (do
    (= (local indent-store)
       (setmetatable {table} {table (kv __mode "v")}))

    (= indent
      (function [n]
       (if [(< n 0) (= n 0)])
       (= (local buf) (at indent-store n))
       (if [buf (return buf)]
           [else (= buf {table})])
       (for [(i 1) n]
         (= (at buf i) "   "))
       (= buf (table-concat buf ""))
       (= (at indent-store n) buf)
       (return buf))))
  ...
  (= codegen
     (function [forms depth]
      ;; TODO: generate sourcemap
      (= (local buf) (concat-gen-form stm sep-with-lines forms (or depth 0)))
      (= (local len) (# buf))
